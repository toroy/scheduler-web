...
import com.zhugeio.platform.scheduler.web.server.constant.JobConstant;
import com.zhugeio.platform.scheduler.web.server.dqc.service.DqcRuleBizService;
import com.zhugeio.platform.scheduler.web.server.dto.AddTaskDto;
import com.zhugeio.platform.scheduler.web.server.dto.ChangeDto;
import com.zhugeio.platform.scheduler.web.server.dto.JobQueryDto;
import com.zhugeio.platform.scheduler.web.server.login.LoginUserDto;
import com.zhugeio.platform.scheduler.web.server.service.inter.JobPageCallback;
import com.zhugeio.platform.scheduler.web.server.vo.JobEnumVo;
import com.zhugeio.platform.scheduler.web.server.vo.JobQueryVo;
import com.zhugeio.platform.scheduler.web.server.vo.VertexBase;
...
import com.zhugeio.platform.scheduler.web.core.dto.JobExtCommonDto;
import com.zhugeio.platform.scheduler.web.core.enums.ErrorCode;
import com.zhugeio.platform.scheduler.web.core.enums.JobPageType;
import com.zhugeio.platform.scheduler.web.core.service.*;
import com.zhugeio.platform.scheduler.web.core.utils.CommonUtils;
import com.zhugeio.platform.scheduler.web.core.utils.JobTypeCache;
import com.zhugeio.platform.scheduler.web.core.utils.StringUtil;
import com.google.common.collect.Lists;
import com.google.common.collect.Maps;
import com.zhugeio.platform.scheduler.web.core.vo.*;
...
public class JobBizService {
	...
	public JobEnumVo getJobEnum(JobPageType type) {
		...
		enumVo.setPriority(Lists.newArrayList(PriorityEnum.values()).stream().map(runEnum -> {
			JobEnumVo.Content content = new JobEnumVo.Content();
		...
		if (type != JobPageType.TASK) {
			List<JobEnumVo.Content> dbSources = this.listDbs(type);
			enumVo.setDbSources(dbSources);
			List<JobEnumVo.Content> dbs = dbSources.stream().map(vo -> {
				JobEnumVo.Content content = new JobEnumVo.Content();
	...
	private void genAlarmNoticeType(JobEnumVo enumVo) {
		...
		List<JobEnumVo.Content> contents = alarmNoticeTypeLists.stream().map(alarm -> {
			JobEnumVo.Content content = new JobEnumVo.Content();
	...
	private List<JobEnumVo.Content> generateRunCount(JobPageType type) {
	...
	private void setJobType(JobPageType type, JobEnumVo enumVo) {
		List<JobEnumVo.Content> contents = Lists.newArrayList();
	...
	private void addCycleType(List<JobEnumVo.Content> contents) {
		for (JobEnumVo.Content content: contents) {
			if (content.isStream()) {
				List<JobEnumVo.Content> subContents = Lists.newArrayList(JobCycleTypeEnum.values()).stream()
				.filter(cycleType -> cycleType == JobCycleTypeEnum.REAL_TIME).map(typeEnum -> {
					JobEnumVo.Content subcontent = new JobEnumVo.Content();
	...
	private JobEnumVo.Content addTypeContent(JobType jobType) {
		JobEnumVo.Content content = new JobEnumVo.Content();
		// PYTHON JAVA HIVE
	...
	private void addChildByJobType(JobPageType type, List<JobEnumVo.Content> contents) {
		...
		for (JobEnumVo.Content content : contents) {
			...
			List<JobEnumVo.Content> subContents = addMachineContent(machines);
			content.setChilds(subContents);
		}
	}

	private List<JobEnumVo.Content> listDbs(JobPageType jobPageType) {
		...
		return allCollectDbVO.stream().map(db -> {
			...
			return content;
		}).sorted(Comparator.comparing(JobEnumVo.Content::getDesc)).collect(Collectors.toList());
	}

	private List<JobEnumVo.Content> ListMachines() {
		List<MachineVO> machineVOs = machineService.listSlaves();
		return addMachineContent(machineVOs);
	}

	private List<JobEnumVo.Content> addMachineContent(List<MachineVO> machineVOs) {
		List<JobEnumVo.Content> contents = Lists.newArrayList();
		...
		// 系统必填
		JobEnumVo.Content sysContent = new JobEnumVo.Content();
	...
	private List<JobEnumVo.Content> listEnums(IEnum[] values) {
