package com.zhugeio.platform.sqlparser.spark

import com.zhugeio.platform.sqlparser.basic.StatementType
import com.zhugeio.platform.sqlparser.entity.*
import com.zhugeio.platform.sqlparser.entity.Function
...
class SparkSqlParserTest {
    ...
    fun createTableSelectTest2() {
        ...
        if (statement is Table) {
            val name = statement.tableName
            Assert.assertEquals(StatementType.CREATE_TABLE_AS_SELECT, statementData.type)
            Assert.assertEquals("users_1", name)
            Assert.assertEquals("select * from users a left outer join address b on a.addr_id = b.id", statement.querySql)
            Assert.assertEquals(2, statement.tableData?.inputTables?.size)
            Assert.assertEquals("address", statement.tableData?.inputTables?.get(1)?.tableName)
        } else {
    ...
    fun setTableLocationTest() {
        val sql = "alter table demo partition(ds='20180317') set location '/user/lineage'"
    ...
    fun queryTest5() {
        val sql = "select \n" +
                ...
                "     left join tidb_datacompute.project_member t2\n" +
    ...
    @Test
    fun insertIntoTest() {
        val sql = "insert into TABLE users PARTITION(ds='20170220') values(\"dfg\")"
        val statementData = SparkSQLHelper.getStatementData(sql)
        val statement = statementData.statement
        if (statement is TableData) {
    ...
    fun insertOverwriteTest() {
        val sql = "insert OVERWRITE TABLE users PARTITION(ds='20170220') values(\"sdv\")"
        val statementData = SparkSQLHelper.getStatementData(sql)
        val statement = statementData.statement
        if (statement is TableData) {
            Assert.assertEquals(StatementType.INSERT_VALUES, statementData.type)
    ...
    fun distcpDruidTest() {
        ...
        val statementData = SparkSQLHelper.getStatementData(sql)
        Assert.assertEquals(StatementType.DISTCP_DRUID, statementData.type)
    }
